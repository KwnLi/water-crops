## RUN SENSITIVITY ANALYSIS WITH DOUBLE RAIN
...
# load in weathergrid
weathergrid <- readRDS("data/weathergrid2.rds")
...
corn.sens.df2 <- dplyr::bind_rows(corn.sens, .id = "soil")
soy.sens.df2 <- dplyr::bind_rows(soy.sens, .id = "soil")

saveRDS(corn.sens.df2,"results-raw/cornsensresults2.rds")
saveRDS(soy.sens.df2,"results-raw/soysensresults2.rds")
